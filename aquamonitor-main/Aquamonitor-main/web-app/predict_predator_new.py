...
def main():
    try:
        ...
        model_path = None
        candidates = [
            os.path.join(os.path.dirname(__file__), 'best.pt', 'best.pt'),
            os.path.join(os.path.dirname(__file__), 'best.pt', 'third_yolo.pt'), 
            os.path.join(os.path.dirname(__file__), 'third_yolo.tflite'),
            os.path.join(os.path.dirname(__file__), 'model.pt'),
            os.path.join(os.path.dirname(__file__), 'best.pt'),
        ...
        for result in results:
            if result.boxes is not None and len(result.boxes) > 0:
                for box in result.boxes:
                    ...
                    # If the model thinks it's a tortoise but it's actually a snake (and vice versa)
                    # we manually swap the display name here.
                    display_name = cls_name.lower()
                    if display_name == 'tortoise':
                        display_name = 'snake'
                    elif display_name == 'snake':
                        display_name = 'tortoise'
                    else:
                        display_name = cls_name # Keep original for others
                    # ------------------------------
        ...
        if not detections:
            ...
            top_detection = detections[0]
            result = {
                'predator': top_detection['class_name'].capitalize(), # Capitalize for clean output
                'confidence': round(top_detection['confidence'], 4)
            }
